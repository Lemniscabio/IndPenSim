...
% BASE EXCLUSION LIST WITHOUT RAMAN_SPEC - RAMAN HANDLED SEPARATELY BELOW
...
% ONLY ADD RAMAN_SPEC TO EXCLUSION LIST IF IT EXISTS IN THE STRUCT - PREVENTS ERROR WHEN RAMAN IS DISABLED
if isfield(Raw_Batch_data.Batch_01, 'Raman_Spec')
    field = [field, {'Raman_Spec'}];
end
...
%      All_variables_to_import = fieldnames(Raw_Batch_data.(Batch_ref)); 
     if ismember('PAA_pred',All_variables_to_import)
         % BASE EXCLUSION LIST FOR PAA_PRED CASE - RAMAN HANDLED CONDITIONALLY
         ...
         % ONLY ADD RAMAN_SPEC IF IT EXISTS - PREVENTS ERROR WHEN RAMAN IS DISABLED
         if isfield(Raw_Batch_data.(Batch_ref), 'Raman_Spec')
             field = [field, {'Raman_Spec'}];
         end
   Temp_batch = rmfield(Raw_Batch_data.(Batch_ref),field);
    All_variables_to_import = fieldnames(Temp_batch); 
